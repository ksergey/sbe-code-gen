...
TEST_CASE("all-types: metadata") {
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeMessageName(), "AllTypesMessage");
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeBlockLength(), 150);
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeTemplateId(), 1);
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeSchemaId(), 1);
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeVersion(), 1);

    REQUIRE_EQ(alltypes::EmptyMessage::sbeMessageName(), "EmptyMessage");
    REQUIRE_EQ(alltypes::EmptyMessage::sbeBlockLength(), 0);
    REQUIRE_EQ(alltypes::EmptyMessage::sbeTemplateId(), 2);

    REQUIRE_EQ(alltypes::DataOnlyMessage::sbeMessageName(), "DataOnlyMessage");
    REQUIRE_EQ(alltypes::DataOnlyMessage::sbeBlockLength(), 0);
    REQUIRE_EQ(alltypes::DataOnlyMessage::sbeTemplateId(), 3);
}

TEST_CASE("all-types: header round trip") {
    auto buffer = makeBuffer();

    auto msg = alltypes::AllTypesMessage::wrapAndApplyHeader(buffer);
    REQUIRE_EQ(msg.position(), 8 + 150);

    auto header = alltypes::MessageHeader{buffer.data()};
    REQUIRE_EQ(header.get<"blockLength">().value(), alltypes::AllTypesMessage::sbeBlockLength());
    REQUIRE_EQ(header.get<"templateId">().value(), alltypes::AllTypesMessage::sbeTemplateId());
    REQUIRE_EQ(header.get<"schemaId">().value(), alltypes::AllTypesMessage::sbeSchemaId());
    REQUIRE_EQ(header.get<"version">().value(), alltypes::AllTypesMessage::sbeVersion());
}

TEST_CASE("all-types: scalars and enums round trip") {
    auto buffer = makeBuffer();
    ...
    msg.get<"mType">().value(alltypes::MessageType::ExecutionReport);
    msg.get<"side">().value(alltypes::Side::Sell);
    msg.get<"bigType">().value(alltypes::BigType::Gamma);
    msg.get<"bits">().value(alltypes::Bits8::On);
    msg.get<"wideEnum32">().value(alltypes::WideEnum32::ValueB);
    msg.get<"wideEnum64">().value(alltypes::WideEnum64::BigB);

    auto header = alltypes::MessageHeader{buffer.data()};
    auto decoded = alltypes::AllTypesMessage{buffer,
    ...
    REQUIRE_EQ(decoded.get<"mType">().value(), alltypes::MessageType::ExecutionReport);
    REQUIRE_EQ(decoded.get<"side">().value(), alltypes::Side::Sell);
    REQUIRE_EQ(decoded.get<"bigType">().value(), alltypes::BigType::Gamma);
    REQUIRE_EQ(decoded.get<"bits">().value(), alltypes::Bits8::On);
    REQUIRE_EQ(decoded.get<"wideEnum32">().value(), alltypes::WideEnum32::ValueB);
    REQUIRE_EQ(decoded.get<"wideEnum64">().value(), alltypes::WideEnum64::BigB);
}

TEST_CASE("all-types: sets and fixed arrays round trip") {
    auto buffer = makeBuffer();

    auto msg = alltypes::AllTypesMessage::wrapAndApplyHeader(buffer);

    auto flags = alltypes::Flags{};
    flags.bit0(true).bit2(true).bit5(false).bit7(true);
    msg.get<"flags">().value(flags);

    auto bigFlags = alltypes::BigFlags{};
    bigFlags.first(true).top(true);
    msg.get<"bigFlags">().value(bigFlags);

    msg.get<"symbol">().value("BTC-USD");

    std::uint32_t history[4] = {1, 2, 3, 4};
    msg.get<"history">().value(history);
    ...
    auto decodedHistory = decoded.get<"history">().value();
    REQUIRE_EQ(decodedHistory.size(), 4u);
    REQUIRE_EQ(decodedHistory[0], 1u);
    REQUIRE_EQ(decodedHistory[1], 2u);
    REQUIRE_EQ(decodedHistory[2], 3u);
    REQUIRE_EQ(decodedHistory[3], 4u);
}

TEST_CASE("all-types: composites round trip") {
    auto buffer = makeBuffer();
    ...
    msg.get<"outerField">().get<"side">().value(alltypes::Side::Sell);
    ...
    REQUIRE_EQ(decoded.get<"outerField">().get<"side">().value(), alltypes::Side::Sell);
...
TEST_CASE("all-types: optional fields") {
    auto buffer = makeBuffer();
    ...
    msg.get<"optSide">().value(alltypes::Side::Buy);
    ...
    REQUIRE_EQ(decoded.get<"optSide">().value(), alltypes::Side::Buy);
...
TEST_CASE("all-types: constant fields") {
    auto buffer = makeBuffer();

    auto msg = alltypes::AllTypesMessage::wrapAndApplyHeader(buffer);
    REQUIRE_EQ(msg.get<"constEnum">().value(), alltypes::MessageType::ExecutionReport);
    REQUIRE_EQ(msg.get<"constStr">().value(), std::string_view{"1.0"});
    REQUIRE_EQ(msg.get<"constNum">().value(), 42);
}

TEST_CASE("all-types: ticks group round trip") {
    auto buffer = makeBuffer();
    ...
    REQUIRE_EQ(msg.position(), 8 + 150 + 2 + 2 * 5);
...
TEST_CASE("all-types: buckets group round trip") {
    auto buffer = makeBuffer();
    ...
    buckets.reset(2);
    REQUIRE_EQ(msg.position(), 8 + 150 + 4);
    uint64_t bucketId = 1;
...
TEST_CASE("all-types: entries group round trip") {
    auto buffer = makeBuffer();
    ...
    entry0.get<"ttype">().value(alltypes::MessageType::NewOrder);
    ...
    entry1.get<"ttype">().value(alltypes::MessageType::ExecutionReport);
    ...
    REQUIRE(decodedFlags.bit0());
    REQUIRE(decodedFlags.bit2());
    REQUIRE_EQ(decodedEntry0.get<"ttype">().value(), alltypes::MessageType::NewOrder);
    ...
    REQUIRE_EQ(decodedEntry1.get<"ttype">().value(), alltypes::MessageType::ExecutionReport);
...
TEST_CASE("all-types: blobRecords group round trip") {
    auto buffer = makeBuffer();
...
TEST_CASE("all-types: variable length data round trip") {
    auto buffer = makeBuffer();
...
TEST_CASE("all-types: compute sizes") {
    REQUIRE_EQ(alltypes::EmptyMessage::sbeComputeSize(), 0);
    REQUIRE_EQ(alltypes::DataOnlyMessage::sbeComputeSize({5}), 5 + 1);
    ...
    REQUIRE_EQ(alltypes::AllTypesMessage::sbeComputeSize(hint), 150 + 6 + 4 + 6 + (2 * 5 + 2) + 1 + 2 + 2 + 4);
}

TEST_CASE("all-types: full round trip sized via sbeComputeSize") {
    auto buffer = makeBuffer();
    ...
    std::size_t const expectedBody = alltypes::AllTypesMessage::sbeComputeSize(hint);
...
TEST_CASE("all-types: empty and data-only messages") {
    auto buffer = makeBuffer();

    auto empty = alltypes::EmptyMessage::wrapAndApplyHeader(buffer);
    REQUIRE_EQ(empty.position(), 8);
    REQUIRE(empty.encodedSize() == 0);
    auto header = alltypes::MessageHeader{buffer.data()};
    REQUIRE_EQ(header.get<"templateId">().value(), alltypes::EmptyMessage::sbeTemplateId());

    auto dataOnly = alltypes::DataOnlyMessage::wrapAndApplyHeader(buffer);
    dataOnly.get<"payload">().value("hello");
    REQUIRE_EQ(dataOnly.encodedSize(), 1 + 5);
    auto dataOnlyHeader = alltypes::MessageHeader{buffer.data()};
    REQUIRE_EQ(dataOnlyHeader.get<"templateId">().value(), alltypes::DataOnlyMessage::sbeTemplateId());
    auto decoded = alltypes::DataOnlyMessage{buffer,
